-- Extend the existing supplier-minimum projection with the canonical
-- supplier-wide pack minimum. Product-level supplier_moq_packs is unchanged.
...
style_sales as (
  select
    mapping.style_id,
    sum(greatest(line.quantity - line.refunded_quantity, 0))::numeric as net_units_sold
  ...
  where orders.cancelled_at is null
    ...
    and orders.shopify_created_at >= sync.completed_at - interval '7 days'
...
select
  ...
  case when sync.completed_at is not null and mapping.mapped_variant_count > 0
    then coalesce(sales.net_units_sold, 0) / 7.0 else null end as average_daily_sales,
  case when sync.completed_at is not null and mapping.mapped_variant_count > 0
    then coalesce(sales.net_units_sold, 0) else null end as average_weekly_sales,
  ...
  case
    when supplier.minimum_order_value = 0 and supplier_rule.minimum_order_packs = 0
      then 'not_applicable'
    when supplier.minimum_order_value > 0 or supplier_rule.minimum_order_packs > 0
      then 'not_satisfied'
    else 'unknown'
  ...
  (
    ...
    and supplier.minimum_order_value = 0
    and supplier_rule.minimum_order_packs = 0
  ) as trusted,
  array_remove(array[
    ...
    case
      when supplier.minimum_order_value > 0 or supplier_rule.minimum_order_packs > 0
        then 'supplier_minimum_order_not_evaluated'
    end,
    case
      when coalesce(supplier.minimum_order_value > 0, false) = false
        and coalesce(supplier_rule.minimum_order_packs > 0, false) = false
        and (
          supplier.minimum_order_value is distinct from 0
          or supplier_rule.minimum_order_packs is distinct from 0
        )
        then 'supplier_minimum_order_unknown'
    end
...
left join public.vault_pack_inventory_intelligence inventory
  on inventory.product_id = style.parent_product_id
 and inventory.colour_design = style.style_name
left join public.vault_product_commercial_intelligence commercial
  on commercial.product_id = style.parent_product_id
left join public.vault_suppliers supplier on supplier.id = style.supplier_id
left join public.vault_supplier_purchasing_rules supplier_rule
  on supplier_rule.supplier_id = supplier.id
