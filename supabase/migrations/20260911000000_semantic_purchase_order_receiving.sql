...
declare
  ...
  already_physically_accounted integer;
...
begin
  ...
  loop
    ...
    select coalesce(sum(
      receipt_line.quantity_received + receipt_line.non_sellable_quantity
    ), 0)::integer into already_physically_accounted
    ...
    if already_physically_accounted + target_quantity + target_non_sellable_quantity > ordered_quantity then
      raise exception 'Physical receipt exceeds the ordered quantity for purchase-order line %', target_line_id;
    end if;
    ...
    loop
      ...
      if not exists (
        ...
        where variant.id = target_variant_id
          ...
          and variant.product_id::text || '::' || trim(variant.model_design) = po_line.style_id
  ...
  select bool_and(received.physically_accounted_quantity = received.ordered_quantity) into all_received
  from (
    select line.id,
      coalesce(line.recommended_units, line.recommended_packs * line.units_per_pack) as ordered_quantity,
      coalesce(sum(
        receipt_line.quantity_received + receipt_line.non_sellable_quantity
      ), 0)::integer as physically_accounted_quantity
    from public.vault_purchase_order_lines line
