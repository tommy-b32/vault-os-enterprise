-- Phase 1B5B: style-level factual size-evidence sufficiency; no curve or buying decision.
create or replace view public.vault_size_evidence_sufficiency as
with historical_bounds as (
  ...
  select
    ...
    (array_agg(a.sizes_observed_current) filter (where a.sizes_observed_current is not null))[1] sizes_observed_current,
    (array_agg(a.sizes_observed_historically) filter (where a.sizes_observed_historically is not null))[1] sizes_observed_historically,
    (array_agg(a.sizes_observed_both) filter (where a.sizes_observed_both is not null))[1] sizes_observed_both,
    (array_agg(a.sizes_observed_current_only) filter (where a.sizes_observed_current_only is not null))[1] sizes_observed_current_only,
    (array_agg(a.sizes_observed_historical_only) filter (where a.sizes_observed_historical_only is not null))[1] sizes_observed_historical_only,
  ...
  from public.vault_size_distribution_evidence_assessment a
  group by a.canonical_style_id
), historical_observations as (
  select
    s.canonical_style_id,
    ...
    min(l.shopify_created_at) historical_first_observed_at,
    max(l.shopify_created_at) historical_latest_observed_at
  from style_evidence s
  join historical_bounds b on b.style_id = s.canonical_style_id
  join public.vault_canonical_resolved_commercial_order_lines l
    on l.parent_product_id = s.parent_product_id
   and l.model_design = s.model_design
   and l.mapping_status = 'resolved'
   and l.shopify_created_at >= b.historical_evidence_start
   and l.shopify_created_at < b.historical_evidence_end
  where s.current_evidence_available
    and s.historical_evidence_available
    and l.net_units > 0
...
select
  ...
  s.parent_product_id,
  s.model_design,
  s.trading_evidence_maturity,
  case
    when not coalesce(s.current_evidence_available, false) or not coalesce(s.historical_evidence_available, false) then 'UNAVAILABLE'
    when coalesce(s.total_historical_observed_units, 0) = 0 then 'NOT_OBSERVED'
    else 'DESCRIPTIVE_ONLY'
  end size_evidence_sufficiency_state,
  s.current_evidence_available,
  s.current_evidence_unavailable_reason,
  s.historical_evidence_available,
  s.historical_evidence_unavailable_reason,
  s.total_current_observed_units,
  s.total_historical_observed_units,
  s.historically_observed_size_count,
  s.currently_observed_size_count,
  s.canonical_size_count,
  s.sizes_observed_current,
  s.sizes_observed_historically,
  s.sizes_observed_both,
  s.sizes_observed_current_only,
  s.sizes_observed_historical_only,
...
from style_evidence s
left join historical_observations o using (canonical_style_id);
