-- Canonical, read-only replenishment inputs for catalogue styles.
-- Sales velocity uses the same seven-day window guaranteed by order reconciliation.
...
select
  ...
  greatest(sync.completed_at, inventory.last_inventory_sync) as freshness,
  case
    when supplier.minimum_order_value = 0 then 'not_applicable'
    else 'unknown'
  ...
  array_remove(array[
    ...
    case when supplier.minimum_order_value is null then 'supplier_minimum_order_unknown' end,
    case when supplier.minimum_order_value > 0 then 'supplier_minimum_order_not_evaluated' end
