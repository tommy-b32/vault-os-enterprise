...
export const StorefrontFunnelRepository = {
  async getToday(): Promise<StorefrontFunnelSnapshot> {
    const { data, error } = await supabaseAdmin
      ...
      .select("tracked_sessions, add_to_cart_sessions, checkout_started_sessions, checkout_completed_sessions, abandoned_checkouts, add_to_cart_rate, checkout_rate, conversion_rate, latest_activity_at")
      .single();
