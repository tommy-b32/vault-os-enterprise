...
import { StoreIntelligence, parseProfitPeriod } from "@/lib/intelligence/StoreIntelligence";
...
export default async function IntelligencePage({ searchParams }: { searchParams: Promise<{ profitPeriod?: string }> }) {
  await requireAuthenticatedOperator();
  const profitPeriod = parseProfitPeriod((await searchParams).profitPeriod);

  let snapshot;
  try {
    snapshot = await StoreIntelligence.getSnapshot(profitPeriod);
  } catch (error) {
    return (
      <VaultAppShell
  ...
  return (
    ...
    >
      <main className="intelligence-page">
        ...
        <section className="intelligence-panel intelligence-insights-panel">
          <div className="intelligence-panel-heading"><div><span>PRODUCT PROFITABILITY</span><h2>{profitPeriod === "all" ? "All-time verified product profitability" : `${profitPeriod.slice(0,-1)} completed days`}</h2></div><div>{(["7d","30d","90d","all"] as const).map(period => <a className={`confidence-badge${period === profitPeriod ? " is-high" : ""}`} href={`/intelligence?profitPeriod=${period}`} key={period}>{period === "all" ? "All Time" : period.toUpperCase()}</a>)}</div></div>
          <div className="trend-metrics"><div><small>Verified contribution</small><strong>{money(snapshot.productProfitabilitySummary.verifiedContribution)}</strong><span>Financially verified sales only</span></div><div><small>Verified revenue</small><strong>{money(snapshot.productProfitabilitySummary.verifiedRevenue)}</strong><span>Includes allocated customer shipping revenue</span></div><div><small>Financial coverage</small><strong>{snapshot.productProfitabilitySummary.verifiedRevenueCoveragePct === null ? "—" : `${(snapshot.productProfitabilitySummary.verifiedRevenueCoveragePct * 100).toFixed(1)}%`}</strong><span>Contribution includes only sales with complete financial evidence. Excluded sales are not treated as zero profit.</span></div></div>
          {snapshot.productProfitability.length ? <div className="weekday-table-wrap"><table className="weekday-table"><thead><tr><th>Product</th><th>Units</th><th>Verified revenue</th><th>Product cost</th><th>Shipping cost</th><th>Payment fees</th><th>Contribution</th><th>Contribution / unit</th><th>Margin</th><th>Financial coverage</th></tr></thead><tbody>{snapshot.productProfitability.map(row => <tr key={row.productId}><td><strong>{row.productName}</strong></td><td>{row.eligibleUnits}</td><td>{money(row.verifiedRevenue)}</td><td>{money(row.cogs)}</td><td>{money(row.shippingCost)}</td><td>{money(row.paymentFees)}</td><td>{money(row.contribution)}</td><td>{row.contributionPerUnit === null ? "—" : money(row.contributionPerUnit)}</td><td>{row.contributionMarginPct === null ? "—" : `${row.contributionMarginPct.toFixed(1)}%`}</td><td>{row.revenueCoveragePct === null ? "—" : `${(row.revenueCoveragePct * 100).toFixed(1)}%`}{row.excludedOrders > 0 ? <small className="confidence-badge">{row.excludedOrders} excluded</small> : null}</td></tr>)}</tbody></table></div> : <p className="intelligence-muted">No financially verified product profitability is available yet.</p>}
        </section>
            ...
            {snapshot.productMomentum.map((product) => (
              <article key={product.title} className={`momentum-card is-${product.status}`}>
                ...
                <div className="model-stock">
                  {product.modelGrouping !== "resolved" ? <><strong>Model grouping unavailable</strong><small>Product-level inventory remains in use.</small></> : (() => { const actionable = product.models.filter((model) => model.attention === "actionable"); const monitors = product.models.filter((model) => model.attention === "monitor"); const informational = product.models.length - actionable.length - monitors.length; const visible = [...actionable, ...monitors]; return <><strong className={actionable.length ? undefined : "is-healthy"}>{actionable.length ? `Actionable model risks — ${actionable.length}` : "No actionable model risk"}</strong>{monitors.length ? <small>Monitoring models — {monitors.length}</small> : null}{informational ? <small>No current model risk — {informational}</small> : null}{visible.sort((a, b) => ({ critical: 5, high: 4, medium: 3, watch: 2, none: 1 })[b.priority] - ({ critical: 5, high: 4, medium: 3, watch: 2, none: 1 })[a.priority]).slice(0, 3).map((model) => <span key={model.key}><b>{model.descriptor}</b> · {model.stock ?? "?"} available · {model.daysCover === null ? "cover unavailable" : `~${model.daysCover}d`} · {model.attention === "monitor" ? "MONITOR" : model.priority.toUpperCase()}{model.constrainedDemand ? ` · DEMAND CONSTRAINED: ${model.constrainedDemand.units} of ${model.sold14} recent units were unavailable` : model.stockImbalance ? ` · STOCK IMBALANCE: ${model.stockImbalance.weakStockShare}% in weak-demand sizes; ${model.stockImbalance.constrainedSizes.join(" / ")} constrained` : model.sizeRisks.length ? ` · ${model.sizeRisks.join(", ")}` : ""}</span>)}</>; })()}
                </div>
              </article>
            ))}
          </div>
...
function IntelligenceStyles() {
  return <style>{`
    ...
    .model-stock{display:grid;gap:4px;margin-top:8px;padding-top:8px;border-top:1px solid rgba(255,255,255,.06)}.model-stock strong{font-size:8px;letter-spacing:.08em;text-transform:uppercase;color:#d9ad43}.model-stock strong.is-healthy{color:#61d08a}.model-stock span,.model-stock small{font-size:8px;line-height:1.35;color:#9ba19e}.model-stock b{color:#e6c25b}
