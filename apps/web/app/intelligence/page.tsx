            ...
            {snapshot.productMomentum.map((product) => (
              <article key={product.title} className={`momentum-card is-${product.status}`}>
                ...
                <div className="model-stock">
                  {product.modelGrouping !== "resolved" ? <><strong>Model grouping unavailable</strong><small>Product-level inventory remains in use.</small></> : (() => { const risks = product.models.filter((model) => model.priority !== "none"); return risks.length === 0 ? <><strong className="is-healthy">Model stock healthy</strong></> : <><strong>Model stock risk — {risks.length} need attention</strong>{risks.sort((a, b) => ({ critical: 4, high: 3, medium: 2, watch: 1, none: 0 })[b.priority] - ({ critical: 4, high: 3, medium: 2, watch: 1, none: 0 })[a.priority]).slice(0, 3).map((model) => <span key={model.key}><b>{model.descriptor}</b> · {model.stock ?? "?"} available · {model.daysCover === null ? "cover unavailable" : `~${model.daysCover}d`} · {model.priority.toUpperCase()}{model.stockImbalance ? ` · STOCK IMBALANCE: ${model.stockImbalance.weakStockShare}% in weak-demand sizes; ${model.stockImbalance.constrainedSizes.join(" / ")} constrained` : model.sizeRisks.length ? ` · ${model.sizeRisks.join(", ")}` : ""}</span>)}</>; })()}
                </div>
              </article>
            ))}
          </div>
