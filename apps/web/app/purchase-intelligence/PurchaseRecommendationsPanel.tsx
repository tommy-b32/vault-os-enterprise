...
type PresentedFixedPackRecommendation = FixedPackPurchaseRecommendationTransport & { productName: string | null; supplierName: string | null };
...
function AddToDraftButton({ styleId, parentProductId }: { styleId: string; parentProductId: string }) {
  ...
  if (purchaseOrderId) return <div aria-live="polite"><strong>Added to Draft PO</strong><Link href={`/purchase-orders/${purchaseOrderId}`}>View Draft PO →</Link></div>;
  return <div aria-live="polite"><button type="button" disabled={pending} onClick={addToDraft}>{pending ? "Adding…" : "Add to Draft PO"}</button>{failure ? <p role="alert">{failure}</p> : null}</div>;
}
