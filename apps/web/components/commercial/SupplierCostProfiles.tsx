...
type Draft = { supplierId: string; costTypeId: string; costTypeName: string; currency: string; exchangeRate: string; packCost: string; shipping: string; importCost: string; units: string; priceUpdatedAt: string };
const emptyDraft = (): Draft => ({ supplierId: "", costTypeId: "", costTypeName: "", currency: "GBP", exchangeRate: "1", packCost: "", shipping: "0", importCost: "0", units: "", priceUpdatedAt: "" });
const draftFrom = (profile: SupplierCostProfileRecord): Draft => ({ supplierId: profile.supplier_id, costTypeId: profile.cost_type_id, costTypeName: profile.cost_type_name, currency: profile.supplier_currency, exchangeRate: String(profile.exchange_rate_to_gbp), packCost: String(profile.pack_cost), shipping: String(profile.shipping_cost_per_pack), importCost: String(profile.import_cost_per_pack), units: String(profile.units_per_pack), priceUpdatedAt: profile.price_updated_at });
...
export function SupplierCostProfiles({ suppliers, profiles }: { suppliers: SupplierPurchasingData[]; profiles: SupplierCostProfileRecord[] }) {
  ...
  return <section className="commercial-card supplier-cost-profiles">
    ...
    <form action={action} className="supplier-minimum-form">
      ...
      {selected ? <input name="supplier_id" type="hidden" value={draft.supplierId} /> : null}<label><span>Supplier</span><select disabled={Boolean(selected)} name="supplier_id" onChange={(event) => update("supplierId", event.target.value)} required value={draft.supplierId}><option value="">Choose supplier</option>{suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.supplier_name}</option>)}</select></label>
      <div className="supplier-minimum-defined-fields">{selected ? <input name="cost_type_id" type="hidden" value={draft.costTypeId} /> : null}<label><span>Canonical cost type ID</span><input disabled={Boolean(selected)} name="cost_type_id" onChange={(event) => update("costTypeId", event.target.value)} pattern="[a-z0-9_]+" placeholder="tee" required value={draft.costTypeId} /></label>{selected ? <input name="cost_type_name" type="hidden" value={draft.costTypeName} /> : null}<label><span>Cost type name</span><input disabled={Boolean(selected)} name="cost_type_name" onChange={(event) => update("costTypeName", event.target.value)} placeholder="Tee" required value={draft.costTypeName} /></label></div>
      ...
      {selected ? <small>Supplier and canonical cost type are locked during edit. A new immutable profile version is recorded when saved.</small> : null}
    </form>
  </section>;
}
