...
function KpiCard({
  ...
  return (
    <article className={`cc-card cc-kpi-card is-${accent}${secondaryValue && secondaryLabel ? " has-secondary" : ""}`}>
      <header><span aria-hidden="true"><VaultIcon name={icon} size={21} /></span><h2>{eyebrow}</h2></header>
      <div className="cc-kpi-value">{value}</div>
      {secondaryValue && secondaryLabel ? <div className="cc-kpi-secondary"><strong>{secondaryValue}</strong><span>{secondaryLabel}</span></div> : null}
  ...
  return (
    <div className="cc-page">
      ...
      <section className="cc-kpi-grid" aria-label="Headline business metrics">
        ...
        <KpiCard eyebrow="Website Traffic" icon="analytics" accent="violet" value={numberValue(data.website.estimatedTotalVisitors)} trend={data.website.visitorTrend} secondaryValue={percentValue(data.website.conversionRate)} secondaryLabel="Conversion">
