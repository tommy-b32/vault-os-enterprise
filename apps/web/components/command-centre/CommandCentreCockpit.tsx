  ...
  return (
    <div className="cc-page">
      ...
      <section className="cc-middle-grid">
        <article className="cc-card cc-feature-panel cc-focus">
          <header className="cc-panel-heading"><div><p>Today&apos;s Focus</p><h2>Highest-priority structured action</h2></div></header>
          {data.todaysFocus.state === "available" ? <div className="cc-focus-body">
            <strong>{data.todaysFocus.title}</strong>
            {data.todaysFocus.description ? <p>{data.todaysFocus.description}</p> : null}
            {data.todaysFocus.unlocks.length ? <small>Evidence · {data.todaysFocus.unlocks.join(" · ")}</small> : null}
            <Link href={data.todaysFocus.destination}>Take action →</Link>
          </div> : <p className="cc-empty">No structured action is currently available.</p>}
        </article>

        <article className="cc-card cc-feature-panel cc-summary">
          <header className="cc-panel-heading"><div><p>Vault Brain</p><h2>Executive summary</h2></div><Link href="/missions">Open Vault Brain →</Link></header>
          <div className="cc-summary-body"><strong>{data.executiveSummary.sentence}</strong>
            {data.executiveSummary.bullets.length ? <ul>{data.executiveSummary.bullets.map((bullet) => <li key={bullet}>{bullet}</li>)}</ul> : null}
          </div>
        </article>
      ...
      <section className="cc-domain-strip" aria-label="Business domain states">
        {data.domains.map((domain) => <div className={`is-${domain.state}`} key={domain.domain}>
          <span>{domain.domain}</span><strong>{domain.state === "not_connected" ? "Not connected" : domain.state.charAt(0).toUpperCase() + domain.state.slice(1)}</strong><small>{domain.detail}</small>
        </div>)}
      </section>
...
function CommandCentreCockpitStyles() {
  return <style>{`
    ...
    .cc-page-header{display:flex;align-items:end;justify-content:space-between;margin-bottom:14px}.cc-page-header p,.cc-panel-heading p,.cc-feed header p{margin:0 0 3px;color:#e2b43e;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.05em}.cc-page-header h1{margin:0;font-size:25px;line-height:1.1}.cc-page-header>div>span{color:#a7aaa6;font-size:12px}.cc-pulse{min-width:180px;padding:9px 12px;border:1px solid rgba(222,176,57,.25);border-radius:10px;background:#101312}.cc-pulse>span,.cc-pulse small{display:block;color:#929894;font-size:9px}.cc-pulse strong{display:block;margin:2px 0;color:#fff;font-size:15px}.cc-pulse.is-critical strong{color:#ff7474}.cc-pulse.is-attention strong,.cc-pulse.is-watch strong{color:#e3b43e}.cc-pulse.is-healthy strong{color:#58d27d}
    ...
    .cc-kpi-grid{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:10px}.cc-kpi-card{position:relative;min-height:128px;padding:13px 14px}.cc-kpi-card header{display:flex;align-items:center;gap:8px}.cc-kpi-card header>span{color:#e4b83f;font-size:17px}.cc-kpi-card h2{margin:0;color:#d4d4d0;font-size:9px;text-transform:uppercase}.cc-kpi-value{margin-top:14px;color:#fff;font-size:24px;font-weight:780;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.cc-change{position:absolute;top:51px;right:13px;font-size:10px;font-weight:700}.cc-change small{display:block;color:#858c87;font-size:7px;font-weight:500}.cc-change.is-positive{color:#4bd17a}.cc-change.is-negative{color:#ff6969}.cc-kpi-support{display:flex;justify-content:space-between;gap:8px;margin-top:13px;color:#9da29f;font-size:9px;line-height:1.35}.cc-kpi-support span:last-child{text-align:right}.cc-kpi-support .cc-traffic-label{max-width:58px}.cc-kpi-support .cc-traffic-breakdown{max-width:132px}
    .cc-middle-grid{display:grid;grid-template-columns:1.05fr .9fr 1.15fr;gap:10px;margin-top:10px}.cc-feature-panel{min-height:188px}.cc-focus{border-top-color:rgba(228,184,63,.65)}.cc-panel-heading{display:flex;align-items:center;justify-content:space-between;padding:10px 12px;border-bottom:1px solid rgba(255,255,255,.07)}.cc-panel-heading h2{margin:0;color:#aeb2ae;font-size:10px;font-weight:500}.cc-panel-heading>a,.cc-panel-heading>span,.cc-feed a,.cc-focus-body>a{color:#d9ae3b;font-size:9px;text-decoration:none}.cc-focus-body,.cc-summary-body{padding:14px}.cc-focus-body>strong,.cc-summary-body>strong{display:block;font-size:12px;line-height:1.35}.cc-focus-body p{margin:6px 0;color:#9fa4a0;font-size:9px;line-height:1.4}.cc-focus-body small{display:block;margin:8px 0;color:#858c87;font-size:8px}.cc-summary-body ul{margin:10px 0 0;padding-left:15px;color:#9fa4a0;font-size:9px}.cc-summary-body li{margin:4px 0}.cc-attention-list{padding:4px 12px}.cc-attention strong{font-size:10px}.cc-attention p{margin:3px 0 0;color:#9fa4a0;font-size:8px;line-height:1.25}.cc-attention{display:grid;grid-template-columns:57px 1fr 13px;align-items:center;gap:8px;padding:7px 0;border-bottom:1px solid rgba(255,255,255,.06);color:inherit;text-decoration:none}.cc-attention:last-child{border:0}.cc-priority{padding:4px;border:1px solid rgba(231,181,54,.4);border-radius:5px;color:#e6ba47;font-size:8px;text-align:center;text-transform:uppercase}.cc-priority.is-critical{border-color:rgba(255,91,91,.5);color:#ff7474}.cc-attention>b{color:#e1b23b}.cc-empty{margin:20px;color:#8f9691;font-size:10px}
    .cc-domain-strip{display:grid;grid-template-columns:repeat(8,minmax(0,1fr));gap:1px;margin-top:10px;overflow:hidden;border:1px solid rgba(219,177,67,.13);border-radius:8px;background:rgba(219,177,67,.13)}.cc-domain-strip>div{min-width:0;padding:7px 8px;background:#0f1211}.cc-domain-strip span,.cc-domain-strip small{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.cc-domain-strip span{color:#858c87;font-size:7px;text-transform:uppercase}.cc-domain-strip strong{display:block;margin:2px 0;color:#dddcd6;font-size:9px}.cc-domain-strip small{color:#767d78;font-size:7px}.cc-domain-strip .is-healthy strong{color:#58d27d}.cc-domain-strip .is-watch strong,.cc-domain-strip .is-attention strong{color:#e3b43e}.cc-domain-strip .is-critical strong{color:#ff7474}.cc-domain-strip .is-not_connected strong{color:#78a8dd}
    ...
    @media(max-width:1200px){.cc-kpi-grid{grid-template-columns:repeat(3,1fr)}.cc-middle-grid{grid-template-columns:1fr 1fr}.cc-middle-grid>article:last-child{grid-column:1/-1}.cc-domain-strip{grid-template-columns:repeat(4,1fr)}.cc-snapshot-grid{grid-template-columns:repeat(2,1fr)}}
    @media(max-width:820px){.cc-middle-grid{grid-template-columns:1fr}.cc-middle-grid>article:last-child{grid-column:auto}.cc-page{padding:16px}.cc-page-header{align-items:flex-start}.cc-pulse{min-width:155px}}
    @media(max-width:600px){.cc-page-header{display:grid;gap:12px}.cc-pulse{width:100%}.cc-kpi-grid{display:flex;overflow-x:auto;scroll-snap-type:x mandatory}.cc-kpi-card{min-width:210px;scroll-snap-align:start}.cc-domain-strip{grid-template-columns:repeat(2,1fr)}.cc-snapshot-grid{grid-template-columns:1fr}.cc-feed-grid{grid-template-columns:1fr}.cc-footer{flex-direction:column;gap:4px}}
  `}</style>;
}
