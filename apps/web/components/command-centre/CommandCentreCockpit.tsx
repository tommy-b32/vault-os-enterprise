  ...
  return (
    <article className={`cc-card cc-kpi-card is-${accent}`}>
      ...
      {comparison && (comparison.state === "available" || comparison.state === "stale") ? (
        ...
        >
          {comparison.value >= 0 ? "+" : ""}{comparison.value.toLocaleString("en-GB", { maximumFractionDigits: 1 })}% <small>{comparisonMeaning ?? "vs comparison"}</small>
        </span>
      ) : null}
  ...
  return (
    <div className="cc-page">
      <header className="cc-page-header">
        <div><p>THE FABRIC VAULT</p><h1>Command Centre</h1><span>Your business at a glance. Make better decisions, faster.</span></div>
        <div className="cc-header-status">
          <time dateTime={data.generatedAt}>{new Intl.DateTimeFormat("en-GB", { weekday: "long", day: "numeric", month: "long", year: "numeric", timeZone: "Europe/London" }).format(new Date(data.generatedAt))}</time>
          <span className={`is-${data.systemStatus}`}>● {data.systemStatus === "live" ? "Live" : data.systemStatus}</span>
        </div>
      </header>
      ...
      <section className="cc-kpi-grid" aria-label="Headline business metrics">
        ...
        <KpiCard eyebrow="Conversion" icon="trend" accent="blue" value={percentValue(data.website.conversionRate)}>
          <span>Sessions {numberValue(data.website.sessions)}</span><span>Add to cart {numberValue(data.website.addToCartToday)}</span>
        </KpiCard>
        <KpiCard eyebrow="Website Traffic" icon="analytics" accent="violet" value={numberValue(data.website.estimatedTotalVisitors)} trend={data.website.visitorTrend}>
          ...
          <span>Live tracked {numberValue(data.website.liveTrackedVisitors)}</span>
        </KpiCard>
        <KpiCard eyebrow="Meta Ads" icon="target" accent="pink" value={display(data.meta.roas, (entry) => `ROAS ${entry.toLocaleString("en-GB", { maximumFractionDigits: 2 })}`)}>
          <span>Spend {moneyValue(data.meta.spend)}</span><span>Revenue {moneyValue(data.meta.attributedRevenue)}</span>
        </KpiCard>
      ...
      <section className="cc-middle-grid">
        <article className="cc-card cc-feature-panel cc-executive-briefing">
          <header className="cc-panel-heading"><div><p><VaultIcon name="brain" size={19} /> Vault Brain Brief</p><h2>Key insights from your commercial intelligence</h2></div><Link className={`cc-confidence is-${data.brainConfidence.state}`} href="/missions">{data.brainConfidence.state === "available" || data.brainConfidence.state === "stale" ? `Confidence ${data.brainConfidence.value.toFixed(1)}%` : "Evidence-led · state-based"}</Link></header>
        ...
        <article className="cc-card cc-feature-panel">
          <header className="cc-panel-heading"><div><p><VaultIcon name="shield" size={18} /> Needs Your Attention</p><h2>Top priorities for today</h2></div><span className="cc-item-count">{data.attention.length} items</span></header>
      ...
      <section className="cc-snapshot-grid" aria-label="Business snapshots">
        ...
        <Snapshot title="Marketing Snapshot" subtitle="Meta Ads · live source" icon="analytics">
          {data.meta.connection === "not_connected" ? (
            <div className="cc-connection-state"><strong>Not connected</strong><p>Connect Meta to make spend, purchases and ROAS live.</p></div>
          ) : null}
        </Snapshot>
...
function CommandCentreCockpitStyles() {
  return <style>{`
    .cc-page{--cc-text-xs:11px;--cc-text-sm:12px;--cc-text-md:14px;--cc-text-lg:16px;--cc-card-padding:18px;--cc-panel-gap:14px;--cc-row-space:11px;min-height:calc(100vh - 72px);padding:26px 30px 20px;color:#f4f2ec;background:radial-gradient(circle at 48% -10%,rgba(75,124,148,.075),transparent 38%),radial-gradient(circle at 70% 26%,rgba(164,83,222,.035),transparent 31%)}
    .cc-page-header{display:flex;align-items:end;justify-content:space-between;margin-bottom:18px}.cc-page-header p,.cc-panel-heading p,.cc-feed header p{display:flex;align-items:center;gap:8px;margin:0 0 5px;color:#f0bd36;font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:.08em}.cc-page-header h1{margin:0;font-size:34px;line-height:1.08}.cc-page-header>div>span{color:#b3b8b7;font-size:14px}.cc-header-status{display:flex;align-items:center;gap:18px;padding:11px 14px;border:1px solid rgba(255,255,255,.09);border-radius:10px;background:rgba(12,16,16,.82);color:#b6bcba;font-size:12px}.cc-header-status span{text-transform:capitalize}.cc-header-status .is-live{color:#59dc87}.cc-header-status .is-stale,.cc-header-status .is-partial{color:#efbe3c}.cc-header-status .is-error{color:#ff6f79}
    ...
    .cc-kpi-grid{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:var(--cc-panel-gap)}.cc-kpi-card{position:relative;min-height:194px;padding:var(--cc-card-padding);overflow:hidden}.cc-kpi-card:after{position:absolute;inset:auto -20% -65% 20%;height:110px;border-radius:50%;background:currentColor;filter:blur(55px);opacity:.035;content:""}.cc-kpi-card header{display:flex;align-items:center;gap:10px}.cc-kpi-card header>span{display:grid;color:currentColor}.cc-kpi-card h2{margin:0;color:#d8dcda;font-size:11px;text-transform:uppercase;letter-spacing:.035em}.cc-kpi-card.is-gold{color:#f0b91f}.cc-kpi-card.is-green{color:#48da7d}.cc-kpi-card.is-blue{color:#54c8f3}.cc-kpi-card.is-violet{color:#9c69ed}.cc-kpi-card.is-pink{color:#ee4da8}.cc-kpi-value{margin-top:16px;color:#fff;font-size:29px;font-weight:780;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.cc-change{position:absolute;top:62px;right:16px;font-size:11px;font-weight:700}.cc-change small{display:block;color:#929894;font-size:11px;font-weight:500}.cc-change.is-positive{color:#4bd17a}.cc-change.is-negative{color:#ff6969}.cc-sparkline{display:block;width:100%;height:38px;margin-top:8px;overflow:visible}.cc-sparkline-empty{height:38px;margin-top:8px;display:flex;align-items:center;color:#69716e;font-size:11px}.cc-kpi-support{position:relative;z-index:1;display:flex;justify-content:space-between;gap:8px;margin-top:8px;color:#9fa7a4;font-size:11px;line-height:1.35}.cc-kpi-support span:last-child{text-align:right}.cc-kpi-support .cc-traffic-label{max-width:92px}
    .cc-middle-grid{display:grid;grid-template-columns:1.45fr 1fr;gap:var(--cc-panel-gap);margin-top:var(--cc-panel-gap)}.cc-feature-panel{min-height:320px}.cc-executive-briefing{border-top-color:rgba(228,184,63,.52)}.cc-panel-heading{display:flex;align-items:center;justify-content:space-between;padding:15px 20px;border-bottom:1px solid rgba(255,255,255,.075)}.cc-panel-heading h2{margin:0;color:#aeb5b2;font-size:12px;font-weight:500}.cc-panel-heading>a,.cc-panel-heading>span,.cc-feed a,.cc-briefing-focus>a{display:inline-flex;align-items:center;min-height:32px;color:#e2b43e;font-size:12px;text-decoration:none}.cc-confidence,.cc-item-count{padding:5px 11px;border:1px solid rgba(77,216,125,.28);border-radius:999px!important;color:#62dd8d!important;background:rgba(77,216,125,.07)}.cc-item-count{border-color:rgba(226,180,62,.3);color:#e2b43e!important;background:rgba(226,180,62,.08)}.cc-confidence.is-unavailable{border-color:rgba(149,158,153,.22);color:#a0a8a4!important;background:rgba(149,158,153,.05)}.cc-briefing-body{padding:18px 20px}.cc-briefing-message{display:grid;grid-template-columns:40px 1fr;gap:14px}.cc-briefing-message>span{display:grid;width:38px;height:38px;place-items:center;border-radius:50%;color:#e3b43e;background:rgba(227,180,62,.12);font-size:20px;font-weight:800}.cc-briefing-message.is-healthy>span{color:#58d27d;background:rgba(88,210,125,.12)}.cc-briefing-message.is-critical>span{color:#ff7474;background:rgba(255,116,116,.12)}.cc-briefing-headline{display:block;font-size:18px;line-height:1.3}.cc-briefing-summary{margin:6px 0 14px;color:#b1b5b1;font-size:13px;line-height:1.45}.cc-briefing-columns{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;padding:14px 0;border-top:1px solid rgba(255,255,255,.07);border-bottom:1px solid rgba(255,255,255,.07)}.cc-briefing-columns>div+div{padding-left:14px;border-left:1px solid rgba(255,255,255,.07)}.cc-briefing-columns b,.cc-briefing-focus>b,.cc-briefing-unlocks>b{display:block;margin-bottom:7px;color:#dbae35;font-size:11px;text-transform:uppercase;letter-spacing:.04em}.cc-briefing-columns ul{margin:0;padding-left:16px;color:#afb3af;font-size:12px;line-height:1.4}.cc-briefing-columns li{margin:4px 0}.cc-briefing-columns span{color:#858d87;font-size:12px}.cc-briefing-focus{display:grid;grid-template-columns:auto 1fr auto;gap:14px;align-items:center;margin-top:14px;padding:11px 14px;border:1px solid rgba(219,177,67,.38);border-radius:8px;background:rgba(219,177,67,.035)}.cc-briefing-focus>b{margin:0}.cc-briefing-focus>strong{font-size:13px}.cc-briefing-focus>span{color:#929994;font-size:12px}.cc-briefing-focus>a{padding:0 8px}.cc-briefing-unlocks{display:grid;grid-template-columns:auto 1fr;gap:12px;margin-top:10px;color:#a4aaa5;font-size:11px;line-height:1.4}.cc-briefing-unlocks>b{margin:0}.cc-attention-list{padding:3px 20px}.cc-attention{display:grid;grid-template-columns:78px 1fr 20px;align-items:center;gap:13px;min-height:65px;padding:10px 0;border-bottom:1px solid rgba(255,255,255,.07);color:inherit;text-decoration:none}.cc-attention:last-child{border:0}.cc-attention strong{font-size:14px;line-height:1.3}.cc-attention p{display:-webkit-box;margin:4px 0 0;overflow:hidden;color:#a8ada9;font-size:12px;line-height:1.4;-webkit-box-orient:vertical;-webkit-line-clamp:2}.cc-priority{padding:7px 4px;border:1px solid rgba(149,158,153,.48);border-radius:6px;color:#b4bbb7;background:rgba(149,158,153,.07);font-size:11px;letter-spacing:-.025em;text-align:center;text-transform:uppercase}.cc-priority.is-critical{border-color:rgba(255,65,65,.82);color:#ff5c5c;background:rgba(255,52,52,.14)}.cc-priority.is-high{border-color:rgba(205,58,78,.72);color:#e86a7d;background:rgba(176,35,55,.13)}.cc-priority.is-medium{border-color:rgba(244,157,45,.68);color:#f3a746;background:rgba(244,145,32,.1)}.cc-priority.is-low{border-color:rgba(151,169,184,.5);color:#b2c1cd;background:rgba(151,169,184,.08)}.cc-priority.is-informational{border-color:rgba(91,169,241,.62);color:#72baff;background:rgba(91,169,241,.1)}.cc-priority.is-unavailable{border-color:rgba(137,145,140,.42);color:#9ba29e;background:rgba(137,145,140,.07)}.cc-attention>b{color:#e1b23b;font-size:20px}.cc-empty{margin:24px;color:#929994;font-size:13px}
    ...
    .cc-snapshot-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:var(--cc-panel-gap);margin-top:var(--cc-panel-gap)}.cc-snapshot{min-height:250px}.cc-snapshot>header{display:flex;gap:11px;align-items:center;padding:14px 17px;border-bottom:1px solid rgba(255,255,255,.07)}.cc-snapshot>header>span{display:grid;color:#e6b632}.cc-snapshot>header>div{min-width:0}.cc-snapshot>header>a{display:grid;margin-left:auto;color:#e6b632}.cc-snapshot h2{margin:0;color:#e6b632;font-size:13px;text-transform:uppercase}.cc-snapshot header p{margin:3px 0 0;color:#9ca29d;font-size:11px}.cc-snapshot-rows{padding:7px 17px}.cc-metric-row{display:flex;justify-content:space-between;gap:12px;padding:9px 0;color:#c5c8c4;font-size:12px;line-height:1.35;border-bottom:1px solid rgba(255,255,255,.05)}.cc-metric-row strong{color:#f0efea;text-align:right}.cc-metric-row strong.is-unavailable,.cc-metric-row strong.is-not_connected{color:#838b85;font-weight:500}.cc-metric-row strong.is-stale{color:#e2b23c}.cc-metric-row small{margin-left:6px;font-size:11px;text-transform:uppercase}.cc-connection-state{display:grid;min-height:170px;place-content:center;text-align:center}.cc-connection-state strong{color:#c0c4c0;font-size:17px}.cc-connection-state p{max-width:230px;margin:9px auto;color:#929a94;font-size:12px;line-height:1.5}
