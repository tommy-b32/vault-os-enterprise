  ...
  return (
    <>
      ...
      <section className="vault-sale-card" role="alert">
        <div className="vault-shopify-mark" aria-label="Shopify order"><span className="vault-shopify-handle" /><span className="vault-shopify-bag">S</span></div>
      ...
      <style>{`
        ...
        .vault-note{position:relative;display:grid;width:82px;height:42px;place-items:center;border:1px solid rgba(86,54,36,.65);border-radius:3px;background:repeating-linear-gradient(12deg,transparent 0 5px,rgba(80,30,20,.08) 6px 7px),linear-gradient(135deg,#d8a188,#f2c7a8 45%,#c7806f);color:#5a3026;font:900 15px/1 Georgia,serif;box-shadow:0 6px 16px #0008;transform:rotate(-4deg);overflow:hidden}.vault-note:before{content:"£";position:absolute;left:7px;top:5px;font-size:9px;opacity:.7}.vault-note:after{content:"BANK OF ENGLAND";position:absolute;right:5px;bottom:4px;font:700 5px/1 Arial,sans-serif;letter-spacing:.05em;opacity:.55}.vault-note-20{background:repeating-linear-gradient(-10deg,transparent 0 5px,rgba(40,20,70,.08) 6px 7px),linear-gradient(135deg,#b9a6bd,#e1d4e2 45%,#9b85a4);color:#44354a}
        .vault-sale-card{pointer-events:auto;position:absolute;z-index:3;top:50%;left:50%;width:min(620px,calc(100vw - 32px));transform:translate(-50%,-50%);padding:28px 34px 22px;border:1px solid rgba(232,188,67,.72);border-radius:12px;background:linear-gradient(145deg,rgba(20,25,23,.98),rgba(7,10,9,.99));box-shadow:0 22px 70px rgba(0,0,0,.65),0 0 38px rgba(232,188,67,.12),inset 0 1px 0 rgba(255,255,255,.05);text-align:center;color:#f5f1e7}
        .vault-shopify-mark{position:relative;width:62px;height:66px;margin:-6px auto 8px;filter:drop-shadow(0 0 18px rgba(149,191,71,.45))}.vault-shopify-bag{position:absolute;left:8px;top:15px;display:grid;width:48px;height:48px;place-items:center;background:linear-gradient(145deg,#a8ce5a,#78a832);clip-path:polygon(12% 5%,88% 5%,100% 100%,0 100%);color:#fff;font:900 32px/1 Georgia,serif;transform:rotate(-3deg)}.vault-shopify-handle{position:absolute;z-index:2;left:20px;top:3px;width:25px;height:24px;border:4px solid #95bf47;border-bottom:0;border-radius:18px 18px 0 0}.vault-sale-kicker{color:#e8bc43;font-size:11px;font-weight:900;letter-spacing:.18em}
        ...
        @media (max-width:600px){.vault-sale-card{top:50%;padding:20px}.vault-sale-title{font-size:22px}.vault-sale-card>strong{font-size:42px}.vault-sale-items{grid-template-columns:1fr}.vault-shopify-mark{transform:scale(.85);margin-bottom:0}}
      `}</style>
    </>
  );
}
