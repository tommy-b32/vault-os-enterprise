  ...
  return (
    <>
      ...
      <style>{`
        ...
        .vault-sale-card-flare{position:absolute;z-index:0;left:50%;top:-22px;width:500px;height:220px;transform:translateX(-50%);background:radial-gradient(ellipse,rgba(255,202,72,.38),rgba(226,150,18,.13) 36%,transparent 70%);filter:blur(10px);pointer-events:none}.vault-sale-sparks{position:absolute;z-index:0;inset:0;overflow:hidden;border-radius:12px;pointer-events:none}.vault-sale-sparks i{--angle:calc(var(--spark) * 25.7deg);position:absolute;left:50%;top:11%;width:3px;height:3px;border-radius:50%;background:#ffd76a;box-shadow:0 0 8px #f2ae20;transform:rotate(var(--angle)) translateY(calc(72px + (var(--spark) % 4) * 18px));opacity:.7}.vault-sale-logo{position:relative;z-index:1;width:112px;height:98px;margin:-12px auto 12px;display:grid;place-items:center;overflow:hidden;filter:drop-shadow(0 0 26px rgba(255,191,43,.42))}.vault-sale-logo img{width:142px;height:142px;object-fit:cover;object-position:50% 35%;transform:scale(1.05)}.vault-sale-kicker{position:relative;z-index:1;display:flex;align-items:center;justify-content:center;gap:13px;color:#f3ce69;font-size:12px;font-weight:900;letter-spacing:.28em}.vault-sale-kicker i{width:46px;height:1px;background:linear-gradient(90deg,transparent,#e8bc43)}.vault-sale-kicker i:last-child{transform:scaleX(-1)}
        ...
        .vault-sale-card>strong{position:relative;z-index:1;display:block;margin:8px 0 2px;color:#54f56f;font-size:66px;text-shadow:0 2px 0 #0b6727,0 0 10px rgba(74,255,111,.95),0 0 26px rgba(42,236,87,.78),0 0 52px rgba(31,196,70,.5);line-height:1.05}
